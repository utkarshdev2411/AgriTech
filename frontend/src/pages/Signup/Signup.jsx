      ...
      <div className='flex bg-gray-100 lg:py-12 flex-col-reverse md:flex-row h-[88vh] items-center justify-center '>
        ...
        <div className='flex w-1/2 justify-center  items-center py-10 flex-col '>
          <form method='post' action='/user/register' className='flex max-w-fit bg-slate-200   flex-col px-16 py-10'>
            ...
            <input className='px-4 mb-4 py-2 bg-transparent rounded-md text-black outline-none border-gray-500 border-[1px]' type="text" id='name' placeholder='Enter your name' value={name} onChange={(e) => { setName(e.target.value) }} required />

            {/* username */}
            <label className='text-md mb-2 font-semibold tracking-wide' htmlFor="username">Username</label>
            <input className='px-4 mb-4 py-2 bg-transparent rounded-md text-black outline-none border-gray-500 border-[1px]' type="text" id='username' placeholder='Enter a username' value={username} onChange={(e) => { setUsername(e.target.value) }} required />
            
            {/* age */}
            <label className='text-md mb-2 font-semibold tracking-wide' htmlFor="age">Age</label>
            <input className='px-4 mb-4 py-2 bg-transparent rounded-md text-black outline-none border-gray-500 border-[1px]' type="number" id='age' placeholder='Enter your age' min={15} max={100} value={age} onChange={(e) => { setAge(e.target.value) }} required />
          ...
          <h1 className='text-sm text-center mt-6 text-gray-500'>Already have an account ?<Link className='font-semibold text-gray-800'> Login here</Link></h1>
        </div>
